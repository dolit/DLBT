...
// 设置对服务器IP进行单独下载限速，单位是BYTE(字节），如果需要限速1M，请输入1024*1024，不设置就使用和其他连接一样的默认限速
...
struct KERNEL_INFO
{
    ...
    UINT64                      totalDownloadedByteCount;       // 总下载的字节数，不包括通讯，是文件数据，包括可能的错误数据（比如我从A用户那获取了一个分块，发现校验失败，也会被记录在内）。
    UINT64                      totalUploadedByteCount;         // 总上传的字节数，不包括上行通讯，是文件的上传
...
struct DOWNLOADER_INFO
{
    ...
    UINT64                      totalDownloadedBytes;           // 下载的数据的字节数，但不是本次启动后的，是累积实际文件完成校验的。比如刚启动，如果本地文件就有500M是上次下载完成后的，那么等文件校验通过，这个数字会从500M开始。
    UINT64                      totalUploadedBytes;             // 上传的数据的字节数，这个是指本次启动后的累积有效上传数据
    ...
    UINT64                      totalBytesDown;                 // 本次启动后，总的所有数据的下行数据量（包括了服务器以及所有客户的数据、辅助通讯数据量等），这个数据因为有很多协议数据，文件下载完成后，给别人上传时的一些下行通讯也会记录在内
																// 所以这个数字随着启动时间，会不停增加
